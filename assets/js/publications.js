(() => {
  ...
  const TOPIC_ORDER = ['LLMs', 'World Models', 'Multi-Agent Systems', 'CV', 'HCI', 'Hashing', 'Time Series'];
  ...
  loadPublications()
    ...
    .catch(() => {
      const fallback =
        '<div class="contact-card"><p>Publications could not be loaded automatically. Please check the publication data files or visit your Google Scholar profile.</p></div>';
  ...
  function isSelf(name) {
    return /your\s+name/i.test(name);
  }
